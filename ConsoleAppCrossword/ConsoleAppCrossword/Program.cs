...
namespace Nonogram
{
    class Program
    {
        ...
        cycleStart:
            ...
            PrintInstructions(rows, userGrid);
        ...
        static void PrintInstructions(List<List<int>> instructions, int[,] arraytoPrint)
        {
            foreach (List<int> row in instructions)
            {
                int listIndex = instructions.IndexOf(row);
                ...
                {
                    Console.Write("{0} ", arraytoPrint[listIndex, i]);
                }
                Console.WriteLine();
            }
        }
